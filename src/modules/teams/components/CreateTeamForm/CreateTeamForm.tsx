...
export const CreateTeamForm: React.FC = () => {
  ...
  const { teamName, userRole, teamInvites } = useSelector((state: RootState) => state.createTeam);

  const handleSendInvites = () => {
    const validEmails = teamInvites.filter(invite => invite.email.trim());
    console.log('Отправляем приглашения:', validEmails);
      ...
      <div className={css.formContent}>
        ...
        <div className={css.inviteSection}>
          ...
          <div className={css.inviteActions}>
            <button
              className={css.sendInvitesBtn}
              onClick={handleSendInvites}
              disabled={!teamInvites.some(invite => invite.email.trim())}
              type="button"
            >
              <Mail size={20} />
